...
async function SendMessage(app, appName, type, status, message, channel) {
  switch (channel) {
    ...
    case 'slack':
      ...
      break;
    case 'ntfy':
      await SendMessageNtfy(app, appName, type, status, message);
      break;
  }
}

async function SendMessageNtfy(app, appName, type, status, message) {
  const env = yenv('oni.yaml', process.env.NODE_ENV)
  const APP = env[app];
  const WEBHOOK_NTFY = APP.WEBHOOK_NTFY;
  const account = APP.APP_ACCOUNT;

  const messageNtfy = status === 'OK' ? `successfully deployed application ${appName} on ${type} in Account: ${account}` : `failed deployed application ${appName} on ${type} in Account: ${account}`;

  try {
    const response = await axios.post(WEBHOOK_NTFY, messageNtfy, {
      headers: {
        "Title": "Pipeline Notification",
        "Tags": status === 'OK' ? "green_circle" : "red_circle"
      }
    });
  } catch (error) {
    console.error('Erro:', error)
